...
class ModelSellerOption extends Model {
	public function addOption($data, $store_id = 0) {
		$this->event->trigger('pre.admin.option.add', $data);

		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "', store_id = '".(int)$store_id."'");
	...
	public function getOption($option_id, $store_id = 0) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE o.option_id = '" . (int)$option_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND o.store_id = '".(int)$store_id."'");

		return $query->row;
	}

	public function getOptions($data = array(), $store_id = 0) {
        $sql = "SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND o.store_id = '".(int)$store_id."'";
	...
	public function getTotalOptions($store_id = 0) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "option` WHERE store_id = '".(int)$store_id."'");
