...
class Cart {
	...
	public function getProducts() {
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				$product_query = $this->db->query("SELECT p.*, pd.*, s.store_id as shop_id, s.name as shop_name, s.url as shop_url, s.key as shop_key FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) INNER JOIN ".DB_PREFIX."product_to_store AS ps ON p.product_id=ps.product_id AND ps.store_id > 0 LEFT JOIN ".DB_PREFIX."store AS s ON ps.store_id=s.store_id WHERE s.status = '1' AND p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = '1' ORDER BY ps.store_id DESC LIMIT 1");
				...
				if ($product_query->num_rows) {
					...
					// Product Specials
					$product_special_query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1");
					...
					$this->data[$key] = array(
						'key'             => $key,
                        'shop_id'         => $product_query->row['shop_id'],
                        'shop_name'       => $product_query->row['shop_name'],
                        'shop_url'        => $product_query->row['shop_url'],
                        'shop_key'        => $product_query->row['shop_key'],
    ...
    public function getShopWeight($shop_id = 0) {
        $weight = 0;

        foreach ($this->getProducts() as $product) {
            if ($product['shop_id'] <> $shop_id) continue;
	...
	public function getSubTotal($shop_id = false) {
		$total = 0;

		foreach ($this->getProducts() as $product) {
			if ($shop_id !== false && $product['shop_id'] <> $shop_id) continue;
            $total += $product['total'];
		}
    ...
    public function getShopSubTotal() {
        $total = array();

        foreach ($this->getProducts() as $product) {
            if (array_key_exists((string)$product['shop_id'], $total)) {
                $total[(string)$product['shop_id']] += $product['total'];
            } else {
                $total[(string)$product['shop_id']] = $product['total'];
            }
        }
	...
	public function hasShipping($shop_id = false) {
		$shipping = false;

		foreach ($this->getProducts() as $product) {
			if ($shop_id !== false && $shop_id <> $product['shop_id']) continue;
