	...
	public function index($_setting = array()) {
		...
		foreach ($modules as $module) {
			...
			if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
				$data['modules'][] = $this->load->controller('module/' . $part[0], $_setting);
			}
						
			if (isset($part[1])) {
				$setting_info = $this->model_extension_module->getModule($part[1]);
                $setting_info = array_merge($setting_info, $_setting);
        ...
        if (preg_match('/^seller\/\w+/i', $route) || preg_match('/^(shipping)|(payment)\/\w+\/edit\/?/i', $route)) {
            $data['modules'][] = $this->load->controller('module/seller');
        }

        if ($route == 'account/login' || $route == 'account/register' || $route == 'account/forgotten') {
            $data['modules'] = array();
        }

        $data['custom_style'] = '';
        //if ($route == 'product/category') {
            $data['custom_style'] = 'width: 20%; padding-right: 5px;';
        //}
