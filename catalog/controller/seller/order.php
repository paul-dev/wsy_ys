...
class ControllerSellerOrder extends Controller {
	...
	public function add() {die('access denied!');
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/order/add', '', 'SSL');
	...
	public function edit() {die('access denied!');
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/order/edit', 'order_id='.$this->request->get['order_id'], 'SSL');
	...
	protected function getList() {
		...
		$data['invoice'] = $this->url->link('seller/order/invoice', '', 'SSL');
		$data['shipping'] = $this->url->link('seller/order/shipping', '', 'SSL');
		$data['add'] = $this->url->link('seller/order/add', '', 'SSL');
		...
		foreach ($results as $result) {
            ...
            foreach ($products as $product) {
                ...
                $order_products[] = array(
                    ...
                    'return'   => $product['return_id'] ? $this->url->link('seller/return/info', 'return_id='.$product['return_id'], 'SSL') : '',
                );
            }
            ...
            $data['orders'][] = array(
				...
				'delete'        => $this->url->link('seller/order/delete', 'order_id=' . $result['order_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_order'] = $this->url->link('seller/order', 'sort=o.order_id' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('seller/order', 'sort=customer' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('seller/order', 'sort=status' . $url, 'SSL');
		$data['sort_total'] = $this->url->link('seller/order', 'sort=o.total' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('seller/order', 'sort=o.date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('seller/order', 'sort=o.date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('seller/order', $url . '&page={page}', 'SSL');
		...
		if (!empty($order_info)) {
			...
			foreach ($products as $product) {
				$data['order_products'][] = array(
					...
					'name'       => $product['name'],
					'model'      => $product['model'],
					'option'     => $this->model_seller_order->getOrderOptions($this->request->get['order_id'], $product['order_product_id']),
					'quantity'   => $product['quantity'],
					'price'      => $product['price'],
					'total'      => $product['total'],
			...
			$data['order_vouchers'] = $this->model_seller_order->getOrderVouchers($this->request->get['order_id']);
    ...
    public function info() {
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('seller/order/info', 'order_id='.$this->request->get['order_id'], 'SSL');
		...
		if ($order_info) {
			...
			$data['shipping'] = $this->url->link('seller/order/shipping', 'order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['invoice'] = $this->url->link('seller/order/invoice', 'order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['edit'] = $this->url->link('seller/order/edit', 'order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$data['cancel'] = $this->url->link('seller/order', $url, 'SSL');
			...
			if ($order_info['customer_id']) {
				$data['customer'] = $this->url->link('sale/customer/edit', 'customer_id=' . $order_info['customer_id'], 'SSL');
			} else {
				$data['customer'] = '';
			}
			...
			$data['payment_address'] = $order_info['payment_address'];
            $data['payment_city'] = $order_info['payment_city'];
            $data['payment_area'] = $order_info['payment_area'];
			$data['payment_postcode'] = $order_info['payment_postcode'];
			...
			$data['shipping_address'] = $order_info['shipping_address'];
            $data['shipping_city'] = $order_info['shipping_city'];
            $data['shipping_area'] = $order_info['shipping_area'];
			$data['shipping_postcode'] = $order_info['shipping_postcode'];
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'href'     		   => $this->url->link('seller/product/edit', 'product_id=' . $product['product_id'], 'SSL')
				);
			}
			...
			foreach ($vouchers as $voucher) {
				$data['vouchers'][] = array(
					...
					'href'        => $this->url->link('sale/voucher/edit', 'voucher_id=' . $voucher['voucher_id'], 'SSL')
				);
			}
			...
			if (1!=1 /*$this->user->hasPermission('modify', 'seller/order')*/) {
				$this->load->model('user/api');
	...
	public function addCommission() {die('access denied!');
		$this->load->language('seller/order');
	...
	public function removeCommission() {die('access denied!');
		$this->load->language('seller/order');
	...
	public function history() {
		...
		$results = $this->model_seller_order->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10);
		...
		$history_total = $this->model_seller_order->getTotalOrderHistories($this->request->get['order_id']);

		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('seller/order/history', 'order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/order_history.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/order_history.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/seller/order_history.tpl', $data));
        }
	}
	...
	public function shipping() {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/order_shipping.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/order_shipping.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/seller/order_shipping.tpl', $data));
        }
	}
	...
	public function delivery() {
        ...
        if (!$this->customer->isLogged() || !$this->customer->isSeller()) {
            $json['error'] = $this->language->get('error_permission');
        } elseif (!isset($this->request->post['status']) || !isset($this->request->post['name']) || !isset($this->request->post['number']) || empty($this->request->post['name']) || empty($this->request->post['number'])) {
            $json['error'] = $this->language->get('error_required_check');
        } else {
            $delivery_data = array(
            ...
            //$delivery_data = serialize($delivery_data);
            ...
            if ($order_info) {
                if (in_array((int)$order_info['order_status_id'], array(2,15))) {
                    if ((int)$order_info['order_status_id'] == 15) {
                        $status = 3;
                    } else {
                        $status = $this->request->post['status'] ? 3 : 15;
                    }
                    $this->load->model('checkout/order');
                    $this->model_checkout_order->addOrderHistory($order_id, $status, $delivery_data, true);
