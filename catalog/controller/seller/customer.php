...
class ControllerSellerCustomer extends Controller {
	...
	public function approve() {die('access denied!');
		$this->load->language('seller/customer');
		...
		if ($customers && $this->validateApprove()) {
			$this->model_seller_customer->approve($this->request->get['customer_id']);
	...
	public function unlock() {die('access denied!');
		$this->load->language('seller/customer');
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('seller/customer/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('seller/customer/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			if (!$result['approved']) {
				$approve = $this->url->link('seller/customer/approve', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL');
			} else {
				$approve = '';
			}			
			...
			if ($login_info && $login_info['total'] > $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('seller/customer/unlock', 'token=' . $this->session->data['token'] . '&email=' . $result['email'] . $url, 'SSL');
			} else {
				$unlock = '';
			}
			...
			$data['customers'][] = array(
				...
				'approve'        => $approve,
				'unlock'         => $unlock,
				'edit'           => $this->url->link('seller/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_email'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&sort=c.email' . $url, 'SSL');
		$data['sort_customer_group'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&sort=customer_group' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$data['sort_ip'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&sort=c.ip' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('seller/customer_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['customer_id'])) {
			$data['action'] = $this->url->link('seller/customer/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('seller/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('seller/customer_form.tpl', $data));
	}
	...
	public function login() {die('access denied!');
		$json = array();
	...
	public function history() {die('access denied!');
		$this->load->language('seller/customer');
	...
	public function transaction() {die('access denied!');
		$this->load->language('seller/customer');
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('seller/customer/transaction', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', 'SSL');
	...
	public function reward() {die('access denied!');
		$this->load->language('seller/customer');
	...
	public function ip() {die('access denied!');
		$this->load->language('seller/customer');
		...
		$results = $this->model_seller_customer->getIps($this->request->get['customer_id'], ($page - 1) * 10, 10);

		foreach ($results as $result) {
			$ban_ip_total = $this->model_seller_customer->getTotalBanIpsByIp($result['ip']);

			$data['ips'][] = array(
				'ip'         => $result['ip'],
				'total'      => $this->model_seller_customer->getTotalCustomersByIp($result['ip']),
				'date_added' => date('d/m/y', strtotime($result['date_added'])),
				'filter_ip'  => $this->url->link('seller/customer', 'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], 'SSL'),
				'ban_ip'     => $ban_ip_total
			);
		}

		$ip_total = $this->model_seller_customer->getTotalIps($this->request->get['customer_id']);

		$pagination = new Pagination();
		$pagination->total = $ip_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('seller/customer/ip', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('seller/customer_ip.tpl', $data));
	}

	public function addBanIp() {die('access denied!');
		$this->load->language('seller/customer');
		...
		if (isset($this->request->post['ip'])) {
			if (!$this->user->hasPermission('modify', 'seller/customer')) {
				...
				$this->model_seller_customer->addBanIp($this->request->post['ip']);
	...
	public function removeBanIp() {die('access denied!');
		$this->load->language('seller/customer');
		...
		if (isset($this->request->post['ip'])) {
			if (!$this->user->hasPermission('modify', 'seller/customer')) {
				...
				$this->model_seller_customer->removeBanIp($this->request->post['ip']);
	...
	public function customfield() {die('access denied!');
		$json = array();
	...
	public function address() {die('access denied!');
		$json = array();
	...
	public function country() {die('access denied!');
		$json = array();
