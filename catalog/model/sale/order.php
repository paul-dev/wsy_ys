...
class ModelSaleOrder extends Model {
    ...
    public function getTotalSellProducts($store_id = 0) {
        if ($store_id == 0) $store_id = $this->config->get('config_store_id');

        $implode = array(2,3,5,15);
        ...
        if ($implode) {
            $query = $this->db->query("SELECT sum(p.quantity) AS total FROM `" . DB_PREFIX . "order_product` AS p INNER JOIN `" . DB_PREFIX . "order` AS o ON p.order_id = o.order_id AND o.parent_id <> 0 AND o.store_id = '".(int)$store_id."' WHERE o.order_status_id IN (" . implode(",", $implode) . ")");
