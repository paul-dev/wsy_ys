...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (preg_match('/^1[1-9]{1}[0-9]{9}$/i', $this->request->post['email'])) {
                ...
                $this->request->post['email'] = $this->request->post['email'] . '@zhubaojie.com';
            } elseif (preg_match('/^[^\@]+@[^\@]+.[a-z]{2,15}$/i', $this->request->post['email'])) {
	...
	public function validate() {
        ...
        /*if (empty($this->request->post['email']) && empty($this->request->post['telephone'])) {
            $this->error['email'] = $this->language->get('error_empty');
            $this->error['telephone'] = $this->language->get('error_empty');
        } else {
            if ((utf8_strlen($this->request->post['email']) > 96) || !preg_match('/^[^\@]+@[^\@]+.[a-z]{2,15}$/i', $this->request->post['email'])) {
                $this->error['email'] = $this->language->get('error_email');
            }
            if (!preg_match('/^1[1-9]{1}[0-9]{9}$/i', $this->request->post['telephone'])) {
        ...
        if (preg_match('/^1[1-9]{1}[0-9]{9}$/i', $this->request->post['email'])) {
            if ($this->model_account_customer->getTotalCustomersByPhone($this->request->post['email'])) {
                $this->error['email'] = $this->language->get('error_exists_phone');
            }
        } elseif (preg_match('/^[^\@]+@[^\@]+.[a-z]{2,15}$/i', $this->request->post['email']) && utf8_strlen($this->request->post['email']) <= 96) {
		...
		foreach ($custom_fields as $custom_field) {
			if ($custom_field['type'] <> 'account') continue;
