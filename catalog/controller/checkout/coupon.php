...
class ControllerCheckoutCoupon extends Controller {
    ...
    public function update() {
        ...
        if (isset($this->request->post['coupon'])) {
            $coupon = (int)$this->request->post['coupon'];
        } else {
            $coupon = 0;
        }

        $coupon_info = $this->model_checkout_coupon->getCouponById($coupon);

        if (!$coupon && $shop_id) {
            //$json['error'] = $this->language->get('error_empty');
            unset($this->session->data['coupons'][$shop_id]);
        } elseif ($coupon_info && $coupon_info['store_id'] == $shop_id) {
            //$this->session->data['coupon'] = $this->request->post['coupon'];
            $this->session->data['coupons'][$coupon_info['store_id']] = $coupon;
